...
export function useChat() {
  ...
  const addAgentMessage = useCallback(
    (text: string, type?: Message["type"]): Message => {
      const msg: Message = {
  ...
  const handleSendMessage = useCallback(
    async (e: React.FormEvent) => {
      ...
      try {
        ...
        addAgentMessage(
          responseText,
          result.is_question ? "question" : "feedback",
        );
      } catch (err) {
        const errorMsg =
  ...
  const handleCsvUpload = useCallback(
    async (file: File) => {
      if (!file || !file.name.endsWith(".csv")) {
        addAgentMessage(
          "Warning: **Invalid file.** Please upload a `.csv` file.",
          "error",
        );
        return;
      }
      ...
      try {
        ...
        toast.success("Strategy dataset processed successfully.");
      } catch (err) {
        setMessages((prev) => prev.filter((m) => m.id !== processingId));
  ...
  const handleCopyMessage = useCallback((text: string) => {
    navigator.clipboard
      .writeText(text)
      .then(() => toast.success("Insight copied to clipboard"))
